package com.android.pkqup.androidnote.util;
...
public class EventMgr<T> {
    ...
    private ConcurrentLinkedQueue<WeakReference<T>> mListenerList = new ConcurrentLinkedQueue<WeakReference<T>>();
    ...
    public void startNotify(INotifyCallback callback) {
        synchronized (this) {
            for (Iterator<WeakReference<T>> iterator = mListenerList.iterator(); iterator.hasNext(); ) {
